...
def print_help() -> None:
    ...
    print("  /eval            Run the 10-question evaluation benchmark suite")
    print("  /history         Show queries asked during this CLI session")
    print("  /exit or exit    Exit the caseworker CLI session\n")


def run_interactive_session(clauses: List[Clause], policy_path: str = "data/policy.md") -> None:
    """
    ...
    while True:
        ...
        if cmd_lower in ("/exit", "exit", "quit", "q"):
            ...
            output = run_grounded_assistant(user_input, clauses)
            print(f"\n{output}")
